program dois_hosts_assexuada
	...
	fenotimo1in = 0.535d0 ! ph2(minimum value)
	fenotimo1f = 0.535d0 ! ph2 (maximum value)
	...
	write(18,*)  "  individuos", "    distancia" , "   repeticao ", "   tempo","   id_linhagem", &
	"   fenotipo", "   sucesso"
	...
	ALLOCATE (Mvar(nind,ngen+2))
			...
			Do while (fenotimo1.le.fenotimo1f)
				...
				Do while (nsucessos_raros.lt.nrep)
					...
					DO n=1, ntempo
						...
						! Checking colonization
			 
						If (n.eq.250) then	
						
							If (M(1,1).ne.0) then
							
								nsucessos_raros=nsucessos_raros+1
								print*, " sucessos =  ", nsucessos_raros
						...
						if (nintroduzidos.gt.0) then
							if(n.eq.200 .or.n.eq.201 .or.n.eq.202 .or.n.eq.203 .or.n.eq.204 .or.n.eq.205 .or.n.eq.206 .or.n&
								.eq.207 .or.n.eq.208 .or.n.eq.209 .or.n.eq.210 .or.n.eq.220 .or.n.eq.230 .or.n.eq.240 .or.n&
								.eq.250 .or.n.eq.300 .or.n.eq. ntempo) then 

								call phenotypic_variability (M, Msub, ngen,  mesp, npar,&
